...
class RCLClient(Client):
    ...
    def setup(self, state_dict, device, local_dataset, global_epoch, local_lr, trainer, **kwargs):
        ...
        self.optimizer = torch.optim.SGD(
            self.model.parameters(), lr=self.base_lr, momentum=self.args.optimizer.momentum, weight_decay=self.args.optimizer.wd
        )
        self.scheduler = torch.optim.lr_scheduler.LambdaLR(
            self.optimizer, lr_lambda=lambda epoch: self.args.trainer.local_lr_decay ** epoch
        )
