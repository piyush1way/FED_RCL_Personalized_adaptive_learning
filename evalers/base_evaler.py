    ...
    def eval(self, model: nn.Module, epoch: int, device: torch.device = None, **kwargs) -> Dict:
        ...
        with torch.no_grad():
            for images, labels in self.test_loader:
                ...
                bin_labels = labels.bincount(minlength=num_classes)
                class_total[:bin_labels.size(0)] += bin_labels.cpu()

                bin_corrects_global = torch.bincount(labels[predicted_global == labels], minlength=num_classes)
                bin_corrects_personalized = torch.bincount(labels[predicted_personalized == labels], minlength=num_classes)
                ...
                for class_idx in range(num_classes):
                    ...
                    if class_mask.sum() > 0:
                        class_loss[class_idx] += this_loss[class_mask].sum().cpu()

                logits_all.append(global_logits.cpu())
                labels_all.append(labels.cpu())
        ...
        acc_global = 100.0 * correct_global / float(total) if total > 0 else 0
        acc_personalized = 100.0 * correct_personalized / float(total) if total > 0 else 0
        
        class_acc_global = torch.zeros(num_classes)
        class_acc_personalized = torch.zeros(num_classes)
        
        for i in range(num_classes):
            if class_total[i] > 0:
                class_acc_global[i] = 100.0 * class_correct_global[i] / class_total[i]
                class_acc_personalized[i] = 100.0 * class_correct_personalized[i] / class_total[i]
        
        loss = loss / float(total) if total > 0 else torch.tensor(0.0)
        
        for i in range(num_classes):
            if class_total[i] > 0:
                class_loss[i] = class_loss[i] / class_total[i]

        balanced_acc_global = class_acc_global.mean().item()
        balanced_acc_personalized = class_acc_personalized.mean().item()
        ...
        results = {
            ...
            "class_acc_global": class_acc_global.tolist(),
            "class_acc_personalized": class_acc_personalized.tolist(),
            "loss": loss.item(),
